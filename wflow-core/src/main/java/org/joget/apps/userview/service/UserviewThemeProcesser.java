...
public class UserviewThemeProcesser {
    ...
    protected String mobileViewRedirection() {
        ...
        if (mobileAgent && !disableMobileView && !desktopCookie) {
            return ("/web/mobile/" + userview.getParamString("appId") + "/" + userview.getPropertyString("id") + "/" + userview.getParamString("key"));
        }
        return null;
    }
    ...
    protected String homePageRedirection() {
        if (userview.getParamString("menuId").isEmpty() && !userview.getPropertyString("homeMenuId").isEmpty()) {
            return "redirect:" + getHomePageLink() + (request.getQueryString() == null ? "" : ("?" + request.getQueryString()));
        }
        return null;
    }
    ...
    protected String loginRedirection() {
        ...
        if (!isAnonymous) {
            String baseLink = getBaseLink().replace("/____/", "//");
            return "redirect:" + baseLink + userview.getParamString("redirectUrl");
        }
        return null;
    }
    ...
    protected String loginPageRedirection() {
        boolean isAnonymous = WorkflowUtil.isCurrentUserAnonymous();
        boolean isAuthorized = (userview.getSetting().getPermission() != null && userview.getSetting().getPermission().isAuthorize());
        boolean hasCurrentPage = userview.getCurrent() != null;
        if ((!isAuthorized || !hasCurrentPage) && isAnonymous) {
            return "redirect:" + getLoginLink() + (request.getQueryString() == null ? "" : ("?" + request.getQueryString()));
        }
        return null;
    }
    ...
    protected String getBodyId() {
        ...
        if (userview.getSetting().getPermission() != null && !userview.getSetting().getPermission().isAuthorize()) {
            bodyId = "unauthorize";
        } else if (isLoginPage) {
            bodyId = "login";
    ...
    protected String getJogetFooter() {
        ...
        if ("true".equalsIgnoreCase(userview.getParamString("isPreview"))) {
            html += "<!--[if IE]><div id=\"preview-label\" class=\"ie\"><fmt:message key=\"fbuilder.preview\"/></div><![endif]-->\n"
    ...
    protected String getBaseLink() {
        return "/web/" + ("true".equalsIgnoreCase(userview.getParamString("embed")) ? "embed/" : "") + "userview/" + userview.getParamString("appId") + "/" + userview.getPropertyString("id") + "/" + userview.getParamString("key") + "/";
    }
