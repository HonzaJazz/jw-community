(function($) {
    ...
    function saveProperties(editor, options){
        ...
        if(pageValidationStack[currentPageId] !== undefined && pageValidationStack[currentPageId]['validators'] !== undefined){
    ...
    function renderPage(id, i, page, options, element, parent){
        ...
        if(page.validators != undefined || page.buttons !== undefined){
        ...
        page.pageId = id + parent + '_' + 'page_' + i;

        html += '</div>' + renderButtonPanel(options, page, true) + '</div>';
    ...
    function renderNoPropertyPage(id, options){
        ...
        html += '</div>' + renderButtonPanel(options, null, false) + '</div>';
    ...
    function renderButtonPanel(options, page, showNavButton){
        ...
        if (page !== null && page.buttons !== undefined && page.buttons !== null) {
            $.each(page.buttons, function(i, button){
                var showHide = "";
                ...
                html += '<input id="'+page.pageId + '_' + button.name+'" type="button" class="page-button-custom" value="'+ button.label +'" data-ajax_url="'+button.ajax_url+'" data-action="'+button.name+'" '+showHide+' />';
                if (button.addition_fields !== undefined && button.addition_fields !== null) {
                    html += '<div id="'+ page.pageId +'_'+ button.name +'_form" class="button_form" style="display:none;">';
                    html += '<div id="main-body-header" style="margin-bottom:15px;">'+button.label+'</div>';
                    $.each(button.addition_fields, function(i, property){
                        html += renderProperty(page.pageId +'_'+ button.name +'_form', i, property, options, "_" + button.name);
                    });
                    html += '</div>';
                }
            });
    ...
    function renderSelectbox(id, property, value, defaultValue){
        var html = '<select id="'+ id +'_'+ property.name +'" name="'+ id +'_'+ property.name +'" class="initChosen">';
        var defaultValueText = '';
        ...
        var options = getOptionsFromCallback(property);
        if (options != null) {
    ...
    function renderMultiselect(id, property, value, defaultValue){
        ...
        var html = '<select id="'+ id +'_'+ property.name +'" name="'+ id +'_'+ property.name +'" multiple'+ size +' class="initChosen">';
        var defaultValueText = '';
    ...
    function renderGrid(id, property, value, defaultValue){
        ...
        $.each(property.columns, function(i, column){
            ...
            var options = getOptionsFromCallback(column);
            if (options != null) {
    ...
    function changePage(currentPage, nextPageId){
        ...
        if(pageValidationStack[pageId] != undefined && pageValidationStack[pageId]['validators'] !== undefined){
    ...
    function changePageAction(pageId, nextPageId){
        ...
        if(validationProgressStack[pageId].count == 0 && validationProgressStack[pageId].valid){
            ...
            $(next).find("select").trigger("chosen:updated");
    ...
    function getData(pagePropertiesDefinition, data, defaultValues) {
        if(pagePropertiesDefinition !== undefined && pagePropertiesDefinition.length !== 0){
            $.each(pagePropertiesDefinition, function(i, property){
                var value = data[property.name];
                var defaultValue = "";
                if(defaultValues !== undefined && defaultValues !== null && defaultValues[property.name] !== undefined){
                    defaultValue = defaultValues[property.name];
                }
                
                if ((value === '' || value === undefined || value === "%%%%%%%%") && defaultValue !== "") {
                    data[property.name] = defaultValue;
                }
                
                if (property.type.toLowerCase() == "password") {
                    data[property.name] = data[property.name].replace(/%%%%/g, "");
                }
            });
        }
        return data;
    ...
    function getErrors(editorId, pagePropertiesDefinition, data, defaultValues, options, parent, validateElementSelect, checkEncryption) {
        var editor = $('div#'+editorId);
        ...
        if(pagePropertiesDefinition !== undefined && pagePropertiesDefinition.length !== 0){
            $.each(pagePropertiesDefinition, function(i, property){
                if (!$(editor).find('#'+editorId+parent+'_'+property.name).hasClass("hidden")) {
                    ...
                    if(property.required !== undefined && property.required.toLowerCase() === "true" && (value === '' || value === undefined || value === '%%%%%%%%' || ($.isArray(value) && value.length === 0) || (property.type.toLowerCase() === "elementselect" && value.className === '')) && defaultValue === ''){
                        var obj = new Object();
                        obj.field = property.name;
                        obj.fieldName = property.label;
                        obj.message = options.mandatoryMessage;
                        errors.push(obj);
                        if(property.type.toLowerCase() === "checkbox" || property.type.toLowerCase() === "radio"){
                            $(editor).find('#'+editorId+parent+'_'+property.name).parent().parent().append('<div class="property-input-error">'+ options.mandatoryMessage +'</div>');
                        }else{
                            $(editor).find('#'+editorId+parent+'_'+property.name).parent().append('<div class="property-input-error">'+ options.mandatoryMessage +'</div>');
                        }
                    }
                    ...
                    if(property.js_validation !== undefined && property.js_validation !== ''){
                        var func = getFunction(property.js_validation);
                        if ($.isFunction(func)) {
                            var errorMsg = func(property.name, value);
                    ...
                    if (checkEncryption && !((value === '' || value === undefined || value === "%%%%%%%%") && defaultValue === '') &&property.type.toLowerCase() === "password") {
                        if ((value.substring(0, 25) === "%%%%****SECURE_VALUE****-") || ((value === '' || value === undefined || value === "%%%%%%%%") && defaultValue !== "")) { 
                            var obj2 = new Object();
                            obj2.fieldName = property.label;
                    ...
                    if(property.type.toLowerCase() === "elementselect" && validateElementSelect){
                        if(elementStack[editorId+parent+'_'+property.name] !== undefined && elementStack[editorId+parent+'_'+property.name].propertiesDefinition !== undefined && elementStack[editorId+parent+'_'+property.name].propertiesDefinition.length > 0){
                            ...
                            $.ajax({
                                url: replaceContextPath(elementStack[editorId+parent+'_'+property.name].url, options.contextPath),
                                context: {
    ...
    function validatePage(editorId, pagePropertiesDefinition, data, defaultValues, parent){
        var options = optionsStack[editorId];
        var errors = getErrors(editorId, pagePropertiesDefinition, data, defaultValues, options, parent, true, false);
    ...
    function attachPageButtonEvent(object){
        //add
        $(object).find('.page-button-custom').click(function(){
            var button = $(this);
            var page = $(button).parent().parent().parent();
            var pageId = page.attr('id');
            var editor = page.parent();
            var editorId = editor.attr("id");
            var id = $(button).attr("id");
            var options = optionsStack[editorId];
            var parent = "";
            
            if ($(page).attr("elementId") !== undefined && $(page).attr("elementId") !== null && $(page).attr("elementId") !== "") {
                parent = $(page).attr("elementId").replace(editorId, "");
            }
            
            //get properties
            var buttonProperties;
            $.each(pageValidationStack[pageId].buttons, function(i, buttonProp){
                if (buttonProp.name === $(button).data("action")) {
                    buttonProperties = buttonProp;
                }
            });
            
            var pageProperties = pageValidationStack[pageId].properties;
            ...
            var data = getPageData(editorId, pageProperties, parent);
            
            //remove previous error message
            $(page).find(".property-input-error").remove();
            var errors = getErrors(editorId, pageProperties, data, options.defaultPropertyValues, options, parent, false, true);
            
            data = getData(pageProperties, data, options.defaultPropertyValues);
            if (errors.length === 0) {
                ...
                if ($("#"+id+"_form").length === 1) {
                    ...
                    $(object).dialog({ 
                        ...
                        buttons: [{
                            ...
                            click: function () {
                                var addition_data = getPageData(id+"_form", buttonProperties.addition_fields, '_' + $(button).data("action"));
                                
                                $("#"+id+"_form").find(".property-input-error").remove();
                                
                                var addition_errors = getErrors(id+"_form", buttonProperties.addition_fields, addition_data, options.defaultPropertyValues, options, '_' + $(button).data("action"), false, false);
                                
                                if (addition_errors.length === 0) {
                                    data = $.extend(data, addition_data);
                                    
                                    executeButtonEvent(data, options, $(button).data("ajax_url"));
                                    $(this).dialog("close");
                                }
                            }
                        }],
    ...
    function executeButtonEvent(data, options, url) {
        $.ajax({
            url: replaceContextPath(url, options.contextPath),
            data : $.param( data ),
            dataType : "text",
            success: function(response) {
                var r = $.parseJSON(response);
    ...
    function initDynamicOptions(object) {
        $(object).find("select.initChosen").each(function(){
            $(this).chosen({width: "54%", placeholder_text : " "});
        });
        $(object).find("select.initFullWidthChosen").each(function(){
            $(this).chosen({width: "100%", placeholder_text : " "});
        });
        ...
        $(object).find("[data-control_field][data-control_value]").each(function() {
            ...
            var controlName = $('[name$=_'+control_field+']:last').attr("name");
    ...
    function getOptionsFromCallback(property) {
        try {
            if (property.options_callback != undefined && property.options_callback != null && property.options_callback != "" ) {
                var func = getFunction(property.options_callback);
                if ($.isFunction(func)) {
                    var options = func(property);
                    if (options != null) {
                        return options;
                    }
                }
