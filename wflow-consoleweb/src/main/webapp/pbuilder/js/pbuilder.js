    ...
    attachTransitionEvent: function (transition, connection) {
        ...
        if (ProcessBuilder.Designer.editable) {
            ...
            $(connection.canvas).bind("click", function(e) {
                e.preventDefault();
                e.stopPropagation();
                var transId = $(this).next(".transition_label").find(".transition_editable").attr("id").substring("transition_".length);
                var transitions = ProcessBuilder.Designer.model.processes[ProcessBuilder.Designer.currentProcessDefId].transitions;
                var selectedTransition;
                for (var t = 0; t < transitions.length; t++) {
                    var ts = transitions[t];
...
ProcessBuilder.Designer = {
    ...
    renderModel: function(model, processDefId) {
        ...
        $("#header_name").remove();
        var $headerName = $("<div id='header_name'>" + appId + ": " + ProcessBuilder.Util.escapeHTML(appName) + "</div>");
...
//        }
    ...
    initParticipants: function(participants) {
        ...
        $participants.find(".node_delete").on("click", function() {
            if (confirm(get_pbuilder_msg("pbuilder.label.confirmDelete"))) {
    ...
    initEditable: function() {
        ...
        $("#process_add").remove();
        var $addButton = $("<span id='process_add'>+</span>");
        ...
        var $editButton = $("<div class='node_edit'><i class='icon-edit'></i></div>");
        var $processes = $(".header_process");
        $processes.find(".node_edit").remove();
        $processes.prepend($editButton);
        ...
        ProcessBuilder.Util.jsPlumb.bind("beforeDrop", function(info) {
            ...
            var sourceId = info.sourceId.substring("node_".length);
            var targetId = info.targetId.substring("end_".length);;
            // check for duplicate transitions to end nodes
        ...
        for (var id in processes) {
            ...
            for (var id in activities) {
                ...
                if (activity.type === 'tool') {
                    ...
                    xml += '<xpdl:SubFlow Execution="' + activity.execution + '" Id="' + activity.subflowId + '">';
            ...
            for (var actId in activities) {
                    ...
                    if (!act.splitTransitions || act.splitTransitions.length === 0) {
                        ...
                        for (var d in act.deadlines) {
                            ...
                            for (var t=0; t<transitions.length; t++) {
                                ...
                                if ($.inArray(pt.id, act.splitTransitions) && pt.type === 'EXCEPTION' && pt.exceptionName === name) {
                                    found = true;
                                    break;
                                }
                            }
                ...
                if (activityInvalid) {
                    ...
                    if (ProcessBuilder.Designer.currentProcessDefId === processId) {
                        ...
                        var message = get_pbuilder_msg("pbuilder.label.missingTransition");
