...
<c:if test="${empty marketplaceContext || marketplaceContext == '???appCenter.link.marketplace.context???'}">
    <c:set var="marketplaceContext" value="/jw"/>
</c:if>
            ...
            success:function(data) {
                ...
                for( var i=0; i<apps.length; i++) {
                    ...
                    for( var j=0; j<userviews.length; j++){
                        ...
                        <c:if test="${isAdmin}">
                            if (!isMarketplace) {
                                content += '<a href="' + baseUrl + contextPath + '/web/console/app/' + app.id + '//forms" onclick="return AdminBar.showQuickOverlay(\'' + baseUrl + contextPath + '/web/console/app/' + app.id + '//forms\')" class="app-edit smallbutton"><i class="icon-edit"></i> <fmt:message key='adminBar.label.designApp'/></div>';
                            }
                        </c:if>
                        content += '<a class="app-link" target="_blank" href="' + baseUrl + uv.url + '">\
                                            <span class="app-icon"><img src="' + baseUrl + contextPath + '/web/userview/screenshot/' + app.id + '/' + uv.id + '" width="240" border="0"></span>\
                                            <div class="app-name">' + UI.escapeHTML(uv.name) + '</div>\
                                            <div class="app-description">' + UI.escapeHTML(app.name) + '</div>\
                                        </a>\
                                    </li>';
                    }
                }
